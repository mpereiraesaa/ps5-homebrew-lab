#!/usr/bin/env bash
set -euo pipefail
echo "deprecated: use build_stage_f.sh <f|g|h|i>; legacy probe builds are frozen" >&2
exit 2
...
build="$root/build-stage-d"
dist="$root/dist-stage-d/PPSA99998"
...
"${cc[@]}" -std=c++20 "${common[@]}" -fno-exceptions -fno-rtti \
  -I"$project" -c "$root/stage_d_main.cpp" -o "$build/obj/main.o"
for unit in stage_b_surface stage_b_compose stage_b_completion stage_b_event_adapter; do
...
"$sdk/bin/prospero-lld" -T "$native/ps5-pie.ld" --eh-frame-hdr \
  ...
  "$build/obj/stage_b_completion.o" "$build/obj/stage_b_event_adapter.o" \
