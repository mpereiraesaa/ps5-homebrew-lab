#!/usr/bin/env bash
set -euo pipefail
echo "deprecated: use build_stage_f.sh <f|g|h|i>; ps5log/1 is mandatory" >&2
exit 2
...
build="$root/build-stage-b"
dist="$root/dist-stage-b/PPSA99998"
...
"${cc[@]}" -std=c++20 "${common[@]}" -fno-exceptions -fno-rtti \
  -I"$project" -c "$root/stage_b_main.cpp" -o "$build/obj/main.o"
for unit in stage_b_surface stage_b_compose stage_b_transaction stage_b_completion stage_b_event_adapter; do
...
"$sdk/bin/prospero-lld" -T "$native/ps5-pie.ld" --eh-frame-hdr \
  ...
  "$build/obj/stage_b_surface.o" "$build/obj/stage_b_compose.o" \
  "$build/obj/stage_b_transaction.o" "$build/obj/stage_b_completion.o" \
  "$build/obj/stage_b_event_adapter.o" --as-needed \
...
python3 "$root/verify_stage_b.py"
