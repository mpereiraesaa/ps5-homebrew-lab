...
#define LOG_PATH "/data/ps5-agc-phase0r-batch-first-submit.log"
...
{
    log_line("PARKED_PHASE0R reason=%s; retain driver, process and mapping; DO_NOT_CLOSE_FAKE00000",
             reason);
    alarm(0);
    for (;;) (void)pause();
}
...
static void watchdog(int signal_number)
{
    ...
    static const char after[] =
        "PARKED_PHASE0R watchdog after submit; retain all; DO_NOT_CLOSE_FAKE00000\n";
    (void)signal_number;
    if (submit_started) {
...
int main(void)
{
    ...
    log_line("phase0R start; BatchMap 0xcf2; 4-byte DMA plus ownership fence; no VideoOut");
    ...
    if (__atomic_load_n(target, __ATOMIC_ACQUIRE) != 0) park("fence completed but target mismatch");
    submit_started = 0;
    log_line("GPU ownership complete fence=0 target=0");
    result = 0;
...
cleanup:
    ...
    log_line("phase0R exit result=%d submitted=%s", result,
