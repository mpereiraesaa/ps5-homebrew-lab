...
Subsequent bounded host translation executed 351 instructions (4096-event limit) from the original
...
at lstrlenA, after LoadStringA reads a private PE string resource, GetStartupInfoA and the original initializer callback with its enclosing _initterm.
Fourteen distinct APIs completed (sixteen calls). Six clock/ID handlers have host
