...
Subsequent bounded host translation executed 104 instructions from the original
...
after GetSystemTimeAsFileTime inside an original-game initializer. Six clock/ID
handlers have host unit tests, with PS5 clock wiring pending. That callback has started
but not returned; full callback evidence remains synthetic. Guest FP control state and CRT mode-pointer getters also
