...
Subsequent bounded host translation executed 85 instructions from the original
Pinball entry plus GetModuleHandleA(NULL), returning the mapped base 0x01000000,
and the CRT state setter __set_app_type, both mode-pointer getters, _controlfp and _initterm.
It now binds 207 imports (205 function tokens, two CRT data words) and stops
at the pending `__getmainargs` API (token 0xe0000520). Initializer callbacks have
translated synthetic evidence, not actual-game callback evidence. Guest FP control state and CRT mode-pointer getters also
